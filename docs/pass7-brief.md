...
| `tools/pass7/` | **new.** The candidates session A judged and adopted — `mermaid-init.candidate.js` and `figure-classes.candidate.css` are now `mermaid-init.js` (committed) and the foot of `custom.css`; `gallery.html` renders one figure of each of twelve kinds in the adopted theme to `render/gallery/`, which is the menu `TEMPLATE.md`'s *kinds* table points at. `break_lane_names.py` puts a long lane's line break at its CamelCase boundary from a render (F17). From session O, `broken_names.py` finds every Mojang name mermaid hyphen-broke on screen, read off `render/index.json` (F18), and `captions.py` lists every figure's caption with its page and line — pass 9's checklist of the captions the session entries did not list | `node tools/render_figures.js --html tools/pass7/gallery.html --out render/gallery`, `python tools/pass7/break_lane_names.py --dry-run`, `python tools/pass7/broken_names.py`, `python tools/pass7/captions.py --missing`, each with `--probe` |
...
it. **`only-child` is the whole of the contract, and session D found it costs
something the standard did not say**: a caption that closes its italics to set a
name and reopens them writes two `<em>`s, stops matching the selector, and loses
its styling and its number — invisibly, in the built page alone. Thirteen were
live when session D looked, **three of them shipped in Part III**, which is to
say the fault survived the session that invented the caption and the two that
wrote sixty of them. The rule is now in `TEMPLATE.md` (one italic run, names in
backticks inside it) and `check_mermaid.js` gates it with a six-case probe. The sentence *before* a figure ends by saying what the figure will show,
...
**Amended again, session H: the commonest way one object gets two lanes is a
class-hierarchy rung drawn beside its own subclass.** Four of Part VIII's five
sequence diagrams did it — `Player` beside `ServerPlayer`, `LivingEntity` beside
`LocalPlayer`, `LivingEntity` beside `ServerPlayer` twice — because the method
being drawn is declared on the base and called on the object, and a lane per
declaring class is the obvious way to say so. It is not one: the object is one
lane, named for the object, and the base's method goes in the *label*
(`SP->>SP: Player.tick, then LivingEntity.tick`). This is worth looking for
first on any page whose subject is an inheritance chain, because it is also
where the part's whole lane budget goes: folding the four took Part VIII's
figures from seven and eight lanes to five and six, and from 0.61–0.69 to
0.80–1.00, without one label being shortened.

**Amended at the close, session O: six lanes.** Measured over the 97 sequence
diagrams as they stand after the part sessions: six lanes never below 0.73 or
11.6px (45 figures), seven at 0.69 and 11.0–11.1px (7 figures, four of them
Part IV's, three Part XI's), eight at 0.60 and 9.7px (3, all Part XI's).
Session E's measurement stands and F7's arithmetic was one lane generous: the
seventh lane is exactly on F1's floor. `TEMPLATE.md` now says six, seven only
where the render says the figure still reads, eight never.
...
**Amended again, session G: the gate now reads a `classDiagram`, which it could
not read at all.** Zero names, zero figures — so the five class diagrams
sessions B to F drew, on F10's own recommendation that the vocabulary pages had
been drawing conversations about objects, had **every name in them unchecked**,
and `pass7_figures.py` was counting the word `classDiagram` itself as a name the
prose never says. Both tools read the kind now: the class name, the display
label of `class Foo["Label"]`, every member line inside the braces, both ends of
a relation and the relation's own label. The convention F12 did not have, because
the kind was invisible to it, is **(e) a member line is qualified by its own
class box** — `int popTime` inside `class ItemStack` is checked as
`ItemStack.popTime`, a failure when no such member exists, and it takes no
`Class.` prefix in the figure. Seven probe cases; no previously-clean part moved
on failures, and all five diagrams turned out to be correct, which nobody knew.
The new **notes** the reading surfaces — relation labels and English member
lines — are session O's to read before `--strict`.

**Amended a third time, session H: the gate could not see a one-word class name,
and could not parse an inheritance relation at all.** Its identifier token
requires two CamelCase humps, so every single-word class in the lane key —
`Entity`, `Avatar`, `Player`, `Mannequin`, `Item`, `Block`, `Mob`, `Screen`,
`Window`, `Font`, `Brain` — was unchecked in every node label, message, note and
class box since the gate was written; only a `participant` line, which is checked
whole, ever caught one. And `CLASS_REL` allowed one character either side of
`--`, so `A <|-- B` matched nothing: both ends unread and the relation's label
unread with them. Both are fixed for the one place the name is **structural
rather than free text** — a `classDiagram`'s `class Foo` box and both ends of a
relation, checked whole the way a lane expansion is, with a box id that carries a
display label read as an *alias* (the label is the name, as with a lane). Free
text is deliberately unchanged: checking every capitalised word in a label would
fail on *Poison*, *Peaceful* and *Normal*. Three probe cases; 43 more names
checked corpus-wide, three more classes reaching the index, and no
previously-clean part moved. **A one-word class in a flowchart node or a message
is still unchecked**, and that is session O's to rule on.

**Amended a fourth time, session I: the gate read only the first 20 kB of a
class file to find what that class extends, and it welded a name to the word
after a `<br/>`.** Exactly one class in the decompile declares itself past the
20,000-character mark — `ClientPacketListener`, at byte 21,133, behind three
hundred import lines — and it is one of the commonest lanes in the book, so the
gate believed it extended nothing and **every inherited member on a `CPL` lane
failed a figure that was right** (`shouldHandleMessage`, which
`ClientCommonPacketListenerImpl` overrides, was the live one). It reads the
whole file now. The second is F18 tripping over its own rule: a part session
told to break a name with `<br/>` writes
`ensureRunningOnSameThread<br/>queues the pair`, and the gate closed that up
into `ensureRunningOnSameThreadqueues`, **so the repair the standard prescribes
made the page fail**. The reading is now F17's own wording — *a name break is
at a CamelCase boundary or at a dot, and nowhere else* — so a break followed by
a lower-case letter, or one whose preceding word is all lower-case (`a tick,
then<br/>MinecraftServer.tickChildren`, which had been quietly hiding a class
name inside `thenMinecraftServer`), is a line break and splits. Four probe
cases; corpus failures 31 → 30 and **notes 166 → 148**, with no previously-clean
part moved. That is the **eighth and ninth** time this pass a part session has
found the gate blind to something the standard cares about, and the second time
the blindness was *created by* a rule the standard itself wrote.

**Amended a fifth time, session J: a qualified member in a message head is
unchecked against the lane it is sent to, and that is where the pass's
commonest fault hides its second spelling.** The gate checks a *bare* head
against its lane — which is how it catches `handleSoundEvent` drawn arriving at
`ClientLevel` — but a head written `Tutorial.onOpenInventory` resolves as a
member of `Tutorial`, so the gate is satisfied and never asks whether `Tutorial`
is the lane. Part X had five of these, all of them **self-messages naming a
third class's method on a lane that owns neither**: `Tutorial.onOpenInventory`
on a `Minecraft` lane, `ServerPlayer.requestDebugSubscriptions` on an `SGPL`
lane, `Mob.registerDebugValues` on a synchronizer's, `BlockItem.place` on a
`ServerLevel` lane, `MouseHandler.releaseMouse` on a `Gui` one. Every one is
F7's *two objects on one lane* wearing F12's clothes, and the honest fixes
differ — sometimes the class is a static façade the lane really calls (`Gizmos`
is, and stays), sometimes the method belongs to the lane after all and the
qualifier is wrong, and sometimes the object needs a lane or the message needs
to go. **A rule cannot be written here without deciding which**, so this is
session O's: the gate could report a qualified head whose class is neither the
lane nor a superclass of it, as a *note*, and a session would rule case by
case. Until then a part session reads its own self-messages by eye and asks of
each: whose method is this, and whose lane is it on?

**Applied at the close, session O — the gate is strict.** `deploy.sh` runs
`check_figure_names.py --strict`, and F12's conventions are failures: an
unqualified member in a node, note, subgraph or edge label (b), a nested class
named without its outer class (c). Three readings were added so that the strict
gate fails only what is wrong: a `classDiagram` relation's label is qualified by
either of its two boxes, and an English member line by its box (read through
the outer class when the box is nested); a word the page sets in *italics* in
its prose is a word in its figures too (the profiler zones on `the-frame`); and
a capitalised word one letter away from a one-word lane-key class fails, which
is session H's question answered without allow-listing anything. Session J's
question is answered as a **note**: a message whose head names a class that is
neither the lane nor its kin. Thirty were listed, **four were the
caller's-method fault in its qualified spelling** (`using-an-item`,
`server-tick`, `fluids`, and `chunk-generation-pipeline`, where the arrow was
also drawn to the wrong object) and were fixed, and twenty-six were ruled:
static helpers the lane calls (`Gizmos`, `RenderSystem`, `Util`, `JsonRpc`,
`EnchantmentHelper`, `VillagerGoalPackages`), objects with no lane of their
own (`Entity.absSnapTo` on a `ClientLevel` lane), and the lane's own inner
class (`LevelCallback` on `ServerLevel`, `ContainerListener` on
`ServerPlayer`, `DialogConnectionAccess` on the client listener). The 119
unresolved names of the planning session's count are **0**, the 362 notes
**62**, and the corpus's last seventeen failures under the new reading were
unqualified members, all fixed by qualifying them. `check_figure_names.py
--probe` has 45 cases.
...
**Done at the close, session O — the thirteen read as one set.** A reader
agent read all thirteen landing pages with their pictures, in watch order.
Every number matched its watch list; the set disagreed with itself on what an
arrow means (eight point from prerequisite to dependent, one the other way,
three are not dependencies at all), what a subgraph means, how a caption says
*numbered to the watch order* (four spellings, two absent), and in seven
sentences that contradicted their own figure. The grammar is now in
`TEMPLATE.md`'s *The landing page*; twelve figures and their captions follow
it, `commands/README`'s downward arrows are the one ruled exception, and Part
XI's figure — the one that fails it outright — is session K's. Two arrows
changed, both claims in [pass9.md](pass9.md): `networking/README` lost the
arrow its own watch list contradicted, and `worldgen/README`'s density-function
arrow points at the first status that samples the field instead of into the
structure box its reading paragraph says never mentions it.
...
**Amended, session J: this is the part sessions' single largest measurable
job, and it is invisible to every gate.** Part X carried **fifteen**
hyphen-broken Mojang names — more than any other part, and more than a third of
the corpus's forty-one — on nine of its thirteen pages: the book spelling a
name wrong in the picture while spelling it right in the prose, three feet
away. None of the six gates sees it, because the source parses, the name
resolves and the label is the right length; **only the render shows it**, and
one regex over `render/index.json` finds every one in a second:

    a text run that ends `[A-Za-z]{3,}-` and is not `--` is mermaid's hyphen

Run that over the part's records *after* the figures are edited, not before:
shortening a message is the first fix (F4) and usually removes the break for
free, and only what survives takes a `<br/>` at a CamelCase boundary or a dot
(F17). A part session should end at zero and say so; the corpus stood at 41
before this session and 34 after it, all of the difference Part X's.
...
**The three are in that order for a measured reason, found by session D**: an
explicit `<br/>` anywhere in a message label turns the theme's 180px wrap *off
for the whole label*, so the rest of the message renders on one unwrapped line
and the figure gets wider rather than narrower. Four such repairs took
`world/lighting`'s trace from 0.69 to **0.55** and its type from 11.1px to
8.8px; removing the breaks and shortening the messages instead put it back to
0.70 with no other change. A break is safe only once the name is the whole
message. `python tools/pass7_figures.py --part <part>` counts the `<br/>` per
figure, and the render is the check.
...
**Re-measured at the close: zero.** Session M's wider reading — a break one
or two letters after a dot or an open parenthesis — is now
`tools/pass7/broken_names.py`, with a probe. It found 20 live (Parts III–V,
closed before session J wrote the check, and two in Part XI); all 20 were
shortened or broken at a boundary, and one of the close's own F12 repairs
broke a twenty-first, which the render caught the same hour.
...
### The close, re-measured (session O, 2026-09-16)

The corpus rendered again after every session but K had run, against the
planning session's numbers of 2026-09-14. **Part XI is the planning session's
Part XI plus ten gate repairs**, so the right-hand column is thirteen parts
finished and one not; Part XI's share of each number is in brackets.

| | before the pass | at the close |
|---|---:|---:|
| figures (mermaid + generated) | 206 (194 + 11) | 224 (211 + 13) |
| median scale | 0.61 | **1.00** |
| below half their size | 76 | **0** |
| shrunk below 0.75 | 122 | 14 [7] |
| showing type under 9px | 89 | **0** |
| showing type under 11px | 107 | 4 [4] |
| widest natural width | 5,415px | 1,809px |
| labels over labels, over shapes, outside their box, clipped | 35 | 2 [2] |
| Mojang names hyphen-broken on screen | 222, after the theme | **0** |
| over 1,200px tall | 42 | 36 |
| captioned | 0 | 203 of 222 [all 19 missing are XI's] |
| opening their section with nothing above | 115 | 79 |
| no lead-in, no caption, no sentence pointing at it | 51 | 8 |
| names the prose never says | 614 tokens on 151 figures | 263 on 109 |
| labels that are sentences | 312 on 121 figures | 69 on 48 [17] |
| sequence diagrams over seven lanes | 7 | 3 [3] |
| flowcharts over fifteen nodes | 9 | 1 |
| figure-name gate: unresolved / notes | 119 / 362 | **0 / 62** |
| figure-kind queue units open | 150 | 28 [17, plus K's handover] |
| mermaid kinds in use | 3 | 4 (ten class diagrams) |
| lane key rows / unused | 342 / 44 | 285 / 2 word lanes |

What the numbers do not show is what the pass was for. **About 180 facts in
figures were corrected** — the sessions' logs count 6, 9, 6, 9, 11, 18, 19, 18,
9, 18, 14, 20 and 9 from A to N, and the close twelve more — on a corpus
fact-checked twice, and the commonest was the same shape in
every part: **a message labelled with the caller's method, drawn arriving at
the callee.** Two fact-checks read every arrow and missed them because a
fact-check reads an arrow's label against the source and an arrow's head
against nothing. The gate reads the head against the lane, which is why the
fault could be found mechanically once the gate existed.

### The verdict

**The pass earned its cost.** Measured, the figures went from a corpus where
the median figure was shown at three-fifths of its size and nine in twenty
showed type a reader could not read, to one where the median is shown whole
and none shows type under 9px; from no captions to two hundred; from a gate that did not exist
to a strict one. Unmeasured, and worth more: the figures were *wrong* far more
often than two fact-checks had found, and wrong in a way the prose beside them
was not — a reader who used the picture as the check on the section, which
is the owner's own description of how the site is read, was being told the
wrong thing on nearly every page. What it cost: fifteen sessions (the planning session, A to J and L to O) and
one to come, and a gate that was blind to something its own standard cared about
**eleven** times — which is the second lesson worth carrying: *a gate written
before the standard it enforces is exercised will be wrong about the
standard; re-probe it every time a session is the first to use a device.*
What it did not do: Part XI (session K, next); the landing pages' length and
the video premise (pass 10); the captions' register and the sentence labels
(pass 8); the 215 long sections with no figure, of which the part sessions
named about fifty as wanting one (pass 10 or the second edition).
...
| **C** | **done 2026-09-15** | III · The server | 6 | 12 | 12 | The part-wide pattern named first here: the lead figures the page cannot be read against — `server-tick`'s lap (fourteen of nineteen names first met there), `server-level-tick`'s, `players-and-sessions`' join sequence (:5176). `server-tick`'s event loop: fourteen edges, node `C` a 24-word sentence in a diamond, two converging edges meaning opposite things (:703, :2085, :5170); `server-level-tick`'s twenty-node vertical chain with a gate clause on every label (:5164) and the profiler zones as prose or as labels (:715); `starting-a-server`: 27 items, the largest single diagram in the part (:706, :2089), a `Worker` lane with no cast row and a `Worker-->>WL` arrow labelled as going to `Main` (:5185); `how-a-server-dies`' `/stop` figure asserting five things explained in the six H3s below it (:5188); `players-and-sessions`' `PLAYER_SPAWN` ticket question (:5262); `server/README`'s part figure in the runtime order rather than the watch order — numbering or a caption (:708); 12 gate failures, among them `DS->>SL: createLevels` and `SL->>CH: broadcastChangedChunks`. **What the session did.** Six viewer agents, one per page. Ten figures became twelve: `server-tick`'s event loop split at `MinecraftServer.pollTaskInternal`'s own joint, and `starting-a-server`'s boot split at the note bar it already drew. Every figure in the part is captioned, pointed at by a sentence, and **legible at the column** — the two below 0.75 are none, the smallest type went 11px → **12.4px**, sentence labels 15 → **0**, and the gate's 12 unresolved names and 13 notes are **0 of each**, which makes Part III the first part clean on the notes as well as the failures. The tallest figure in the book fell **6,010px → 2,311px**: `server-level-tick`'s twenty-two-step chain lost the gate clause appended to every label to a three-column table beside it, and gained the one containment it was hiding — six of its boxes are the insides of `ServerChunkCache.tick`, which is why the prose's *five things* and the figure's six boxes had been disagreeing since pass 5. **The part's real fault was not the one the queue named.** Under *the lead figure the page cannot be read against* sat twelve messages — one on every page — **labelled with the caller's method and drawn arriving at the callee**: `MinecraftServer.saveAllChunks` on an arrow into `ServerLevel`, `ServerLevel.sendBlockUpdated` into `ServerChunkCache`, `ServerChunkCache.broadcastChangedChunks` into `ChunkHolder`, and nine more. Every one was a gate failure and every one was a claim about who does what; the gate finds the shape mechanically, and most of the 88 corpus failures left look like it. Six corrections, all in [pass9.md](pass9.md); the sharpest is `how-a-server-dies`' watchdog figure, which had no `Disk` lane and so said *the watchdog writes nothing* beside a table on the same page whose row reads *is a crash report written: yes*. Also settled: the queue's open question about what holds a joining player's chunks before `PrepareSpawnTask.Ready` — nothing has to, because `TicketType.PLAYER_SPAWN` carries `FLAG_LOADING` alone and its twenty-tick timeout does not start while the chunks are still loading. Also fixed: **`check_figure_names.py` could not read the tick band F8 tells every part session to draw** — Part III drew the corpus's first four `rect` bands and the gate read `rgba(0, 0, 0, 0.04)` as a call named `rgba`; one rule and two probe cases, the second tool blindness a part session has found in two sessions. |
| **D** | **done 2026-09-15** | IV · The world | 11 | 24 | 10 | The largest part by figures: 24, of which 18 open their section, 17 introduce most of their names first, seven are over 1,200px tall, and 19 names fail the gate. Three lead figures the page cannot yet be read against — `chunk-storage`'s flowchart, `chunk-generation-pipeline`'s pyramid with radius 11 on its `EMPTY` node, `fluids`' bucket sequence (:367); `environment-attributes-and-timelines`' `flowchart BT` under a heading that says the value *falls* (:5020 — the figure is the cheaper end, the heading is an anchor seven pages land on); `game-events-and-vibrations`' fifteen-node gate flowchart with edge labels of five and six conditions (:378, 2,395px tall) and its trace's one lane for the block and its block entity (:381); `lighting`'s batch flowchart missing `checkNode` (:385); `scheduled-ticks`' `LevelChunk` lane that decides nothing (:388); `chunk-anatomy`'s two graphs over five names (:393) and its section figure without the light (:472); `world/README`'s conveyor drawn as a cycle (:390); whether `points-of-interest`'s and `tickets-and-loading`'s secondary state diagrams are the page's true picture (:3167). **What the session did.** Eleven viewer agents, one per page. Twenty-four figures became **twenty-seven**: four splits at a mechanism's own joint (`game-events`' 24-node cascade, `fluids`' three-branch tick, `scheduled-ticks`' pipeline, and `chunk-generation-pipeline`'s twelve-status chain, which became a four-column table and a new nested-ring pyramid). **No type under 9px anywhere in the part** (from 2), **no figure over fifteen nodes** (from 1), no lane over seven, the widest figure 4,978px tall → 2,077px, and the gate's **19 unresolved names and 63 notes are 0 of each**. Every one of the 27 figures is captioned and every non-lead figure has a lead-in. `chunk-anatomy`'s lead figure is the book's third `classDiagram` — the section's heading promised four shapes and the paragraph under it stated a hierarchy the old figure did not draw at all. **The part-wide fault was Part III's, again**: fourteen of the nineteen gate failures were a message labelled with the caller's own method and drawn arriving at the callee, which makes it four parts of four, and the gate finds it mechanically. **The session's own finding is about the standard rather than the pages**: a caption that closes its italics to set a name and reopens them has two `<em>` children, stops matching `custom.css`'s `p:has(> em:only-child)`, and silently loses its styling and its figure number in the built page — **thirteen were live, three of them shipped in Part III**, and neither `check_mermaid.js` nor the markdown ever showed it. All thirteen fixed, the rule is in `TEMPLATE.md`, and `check_mermaid.js` gates it with a six-case `--probe`. Two more render traps recorded: an explicit `<br/>` turns the theme's wrap off for its whole label (four such “repairs” took `lighting`'s trace from 0.69 to 0.55), and a subgraph title is clipped to one wrapped line. Nine corrections, all in [pass9.md](pass9.md); the sharpest is `environment-attributes-and-timelines`, where the stack figure drew the two lightning-flash layers as rungs every value passes and `ClientLevel` bolts each to **one named attribute**, so they are absent from the other forty-six. Also: `check_figure_names.py` welded two names into one wherever a node label broke a line after punctuation — one rule, two probe cases, the third tool blindness a part session has found in three sessions. Eleven queue entries struck with the page open, seven opened, and the open question about a secondary state diagram answered for both its remaining pages. |
| **E** | **done 2026-09-15** | V · Blocks | 8 | 10 | 11 | **The flag words** (:5325): four lead figures label their gates with bare flag numbers the page has not minted; session E fixed the prose half and this session decides whether a gate may carry a constant's name (F4), on `blocks-and-states`' write figure first — 3,766px tall, 21 nodes, ten sentence labels, the most-linked figure in the part, two subgraphs already (:1942, :5333: no paragraph beside its build-time figure, which introduces four classes the cast omits). `block-interaction`'s sequence: eight arrows that are later sections in shorthand (:5337); `block-entities`' three names for one menu (:4071) and `ServerChunkCache.blockChanged` on the `ServerLevel` lane (:5341); `signal-and-dust`'s sequence showing only the *on* case where the hook is the line going dark — a third dust (:5344); `diodes-and-observers`' channel flowchart re-deriving the channels in its labels (:4074, :5350 — the figure was the only definition on the page); `pistons-and-block-events`' flag table wanting the wide treatment (:4069); 13 gate failures. **What the session did.** Eight viewer agents, one per page. Ten figures became **thirteen** — three pages split one figure at a joint the prose already names, and the part gained the book's **fourth `classDiagram`** on `blocks-and-states`, whose twelve-node flowchart had been drawing inheritance and construction with the same arrow. All thirteen are captioned; **no figure in the part shows type under 11.9px** (five were at 10.3–11.1px), none is below 0.74, sentence labels went 38 → **0**, bare-number gates 3 figures → **0**, and the gate's **13 unresolved names and 26 notes are 0 of each**. The part's most-linked figure — the write, 21 nodes and 3,409px with a gate clause appended to ten of its labels — is two figures split at the re-read plus a thirteen-row *step · side · flags · also needs* table, which is session C's `server-level-tick` remedy applied a second time and the only one that works on this shape. **The part-wide fault was Parts III and IV's, a third and fourth time**: thirteen of the thirteen gate failures were a message labelled with the caller's own method and drawn arriving at the callee — `Minecraft.startAttack` into `MultiPlayerGameMode`, `ServerLevel.doBlockEvent` into `PistonBaseBlock`, `LeverBlock.updateNeighbours` into `ServerLevel` — which makes it **five parts of five, and every one of them found mechanically**. **The session's own finding is about the seven-lane limit**: four of Part V's five sequence diagrams were at seven lanes, and seven lanes is 1,650px, which is 0.66 at the column and 10.6px of type — so F7's *at most seven* and F1's *nothing under 11px* are **not both satisfiable**, and the fix is never a shorter label (shortening eight of them moved the width by zero) but one lane fewer or a split. Every one of those four is now six lanes or two figures, and `block-interaction`'s client half is at 0.97. Eleven corrections, all in [pass9.md](pass9.md); the sharpest is `block-breaking`, where the figure printed the server's progress as **1.07** and the prose two sections below says both sides *arrive at the same 1.064* — the one number the page exists to say is shared, printed two ways inside one picture. Also settled: the flag-word question the queue left for this session was already answered by F4 (a gate carries the constant's name; `flag 2` only where the page has paired name and number above the figure), and applying it found `blocks-and-states`' lead-in promising **seven** bits and naming **eight**. |
| **F** | **done 2026-09-15** | VI · Entities | 10 | 22 → **26** | 12 | Three pages split one figure at a joint the prose names — `synched-entity-data`'s shear at the tick, which is what gave the two containers a lane each; `entity-anatomy`'s summon at the same boundary, so the page's title happens twice; `authority`'s boat by machine. `entity-anatomy`'s lead flowchart is the book's **fifth `classDiagram`** (eleven boxes to three); `damage-and-death` traded a three-screen chain for a short one carrying the running number and a nine-row table, and gained a figure for the veto in `LivingEntity.die`. **No type under 12.6px**, nothing below 0.79, sentence labels 14 → **2**, every figure captioned, the figure gate's **14 unresolved → 0**. **The caller's-method-at-the-callee fault, a sixth time** (eleven of the fourteen), and on three pages fixing it *removed lanes* — `movement-and-collision` seven → three. **Its own finding: a flowchart or state label beginning `1. ` is a markdown list, so mermaid replaces it with the string *Unsupported markdown: list* — nine were live, five of them six nodes of one Part XI figure**; `check_mermaid.js` gates it with a nine-case probe. F13's tree emitter done for all five trees at once (1,619–2,217px → 810–1,020px; the `Entity` tree 0.62/7.5px → **1.00/12px**). Eighteen corrections, ten queue entries struck, five opened. |
| **G** | **done 2026-09-15** | VII · Items and inventories | 9 | 17 → **19** | 15 | The most queue units of any part. `items/README`'s figure: a chain where the prose claims two tiers, an orphan node that is true and reads like an omission, no edge between `EC` and `LO` (:1747, :4367, :4405 — a landing figure numbered to the watch order, F13); `containers-and-menus`' shift-click sequence before every paragraph that explains it, off by one step against the prose (:4330), and its ladder flowchart restating the trace (:4339); `loot-tables`' opening sequence with no client lane and an arrow that reads the wrong way (:4342), its sixteen-edge funnel and the note that carries a correction (:4414), and *three fan-outs* over four (:1246); `items-and-stacks`' `ItemStack` figure with edgeless `count` and `popTime` and the dotted arrow to a node defined two lines down (:4363), and the same object drawn on `data-components` (:4409); `enchanting`'s 50 that appears only in the figure (:4360); `enchantments`' hook flowchart omitting the flag row the paragraph points at (:4352); `recipes`' three senses of *enabled* (:4356); `using-an-item`'s two disconnected graphs in one box (:4348); `contexts-and-predicates`' stray node (:1752); `the-spear`'s label compressing two sides into one (:3568). **What the session did.** Nine viewer agents, one per page. Seventeen figures became **nineteen**: four splits at a joint the prose already names — `using-an-item`'s *ending*, which was two disconnected graphs in one box and is now the page's two endings; `recipes`' trace at its own tick bar; `containers-and-menus`' seventeen-node ladder, into a four-row table and a five-node fork; and `loot-tables`' funnel, which lost its placement tail to the trace above it and its five entry containers to a table (26 nodes → 14). The book's **sixth `classDiagram`** is here (`items-and-stacks`: four edgeless boxes and a subgraph title clipped mid-phrase on screen), and `enchanting`'s trace is split by machine with a lane each for the two copies of `EnchantmentMenu`. Every figure captioned and pointed at, **nothing below 0.79 and no type under 12.6px**, no lane over six, no flowchart over fourteen nodes, and the gate's **10 unresolved names and 31 notes are 0 and 1** (the one ruled: `LootItemCondition.test` is `java.util.function.Predicate`'s). Nineteen corrections, all in [pass9.md](pass9.md), and **fifteen of them the caller's-method-at-the-callee fault — six parts of six**; the sharpest is `loot-tables`' opening trace, two lanes for one object with the content packet returning to `ServerPlayer`. **The session's own finding is the fifth tool blindness, and the one F10 walked into**: `check_figure_names.py` read **nothing at all** inside a `classDiagram` — zero names, zero figures — so the five class diagrams sessions B to F drew on F10's own recommendation had every name in them unchecked, and `pass7_figures.py` counted the word `classDiagram` itself as a name the prose never says. Both read the kind now, with a **member line checked against its own class box** the way a message is checked against its lane; seven probe cases, no previously-clean part moved. Beside it, **nine Mojang names the theme was hyphen-breaking on screen** (F18), every one of them the book spelling a name wrong in the picture and right in the prose — and all nine found **mechanically**, because `render/index.json` carries the on-screen text of every label, so a break is one regex over the render. Fifteen queue entries struck, one ruled out, twelve opened. |
...
| **O** | **done 2026-09-16** (before K) | the close | all | 224 | 150 → **28** | **The gate flipped**: every one of the 119 unresolved names corrected or ruled, the F12 conventions applied so the notes are failures, `deploy.sh` runs `check_figure_names.py --strict`, and `CLAUDE.md`'s rule 5 says the figures are under it; `check_mermaid.js` refuses `%%{init}%%`, `classDef`, `style` and `linkStyle` in a page (F2); the lane key pruned to lanes in use and `PTT`'s replacement chosen, `check_lanes.py --unused` in `deploy.sh`; the theme's final form and the caption counter; `TEMPLATE.md`'s *Figures* and *Lanes* read against the finished parts; the thirteen landing figures read as one set, numbered to the watch order and captioned; the corpus re-rendered and re-measured against Part 4's table (scale, type, overlaps, captions, names first met, the gate) and the difference written down; the pass's own strikes audited (a strike is a claim); [pass9.md](pass9.md)'s entries checked for shape — every redrawn figure listed with its orderings; the verdict on whether the pass earned its cost; pass 8's charter detail written with the wording debt the figures raised (captions' register, label wording) counted afresh; this file's Part 5 left for the pass-8 planning session to archive. | **What the session did.** Ran before K, at the owner's word, so Part XI is the one part it could not close; it made ten gate repairs there and left the rest to K. **The gate is strict** (`deploy.sh`, `CLAUDE.md` rule 5): F12(b) and (c) fail, class-diagram boxes qualify their labels, a page's italic words are words, a one-letter misspelling of a one-word key class fails, and a qualified head on a lane that owns neither is a note — thirty listed, four fixed as the caller's-method fault, twenty-six ruled. Seventeen unqualified names qualified; 0 unresolved, 62 notes. **`check_mermaid.js` fails colour in a page** (eleven probe cases). **The lane key lost 67 rows**; `CMTE` replaces `PTT` as the example; `--unused` reports in `deploy.sh`. **Twenty names broken on screen → 0** (`tools/pass7/broken_names.py`). **The thirteen landing figures read as one set** by a reader agent, their grammar written into `TEMPLATE.md`, twelve fixed, seven sentences that contradicted their own figures corrected, two arrows changed, XI left to K. **The exemplar's spawn loop** gained the two group-ending arrows session B left to it. **Rulings:** six lanes, `par`/`loop`/`alt`/`opt` and a call's subgraph in the marks table, no colour or width knob for class diagrams, subgraph `direction TB` allowed, the landing budget counts prose, the tier front doors need no role, the dependency figure keeps all 27 arcs. **The audits:** 122 strikes, 4 not borne out and amended, one unstruck; 13 `pass9.md` entries, 9 not walkable item by item — the form is now in its head, and `tools/pass7/captions.py` lists the 203 captions no entry did. The corpus re-measured above, the verdict above, pass 8's charter detail in [plan.md](plan.md). |
