...
holding each one's priority table. → [AI](../entities/ai-goals-and-brains.md)
...
asks the aquifer which block to write. → [the worldgen pipeline](../worldgen/worldgen-pipeline.md)
...
in the End — off a single erosion sample. → [biomes](../worldgen/biomes.md)
...
mirrored to nearby clients as a packet. → [redstone](../blocks/redstone.md)
...
frame. → [the frame](../rendering/the-frame.md)
...
which does not ask. → [the worldgen pipeline](../worldgen/worldgen-pipeline.md)
...
radius that step needs. → [the chunk generation pipeline](../world/chunk-generation-pipeline.md)
...
packet listener at a time, swapped when the protocol phase changes. → [the connection](../networking/the-connection.md)
...
damage calculation and death message reads. → [damage and death](../entities/damage-and-death.md)

**DataLayer** — the nibble array one section's block light or sky light lives
in, owned by the light engine and never by the section. → [lighting](../world/lighting.md)

**Debug subscription** — a registered kind of debug value a client can ask
the server for; the server polls it, diffs it, and sends only what changed. → [debugging the running game](../client/debugging-the-running-game.md)
...
loot predicates; the enchantment itself never crosses the wire. → [enchantments](../items/enchantments.md)
...
schedule. → [environment attributes and timelines](../world/environment-attributes-and-timelines.md)
...
fresh immutable values. → [the frame](../rendering/the-frame.md)
...
is asked for. → [text and fonts](../client/text-and-fonts.md)
...
it hands out. → [Blaze3D](../rendering/blaze3d.md)
...
frame. → [the GUI render tree](../client/the-gui-render-tree.md)
...
recipe holding it is never placeable. → [recipes](../items/recipes.md)
...
would be wrong. → [items and stacks](../items/items-and-stacks.md)
...
client, sharing an abstract `Level` and remarkably little else. → [the level tick](../server/server-level-tick.md), [the client level](../client/the-client-level.md)
...
**re-parsed** per distinct argument tuple, cached only eight deep. → [execution and functions](../commands/execution-and-functions.md)
...
materialises the structure it walks through on a write. → [scores, teams and stored data](../commands/scoreboard-and-data.md)
...
driven by the game itself. → [scores, teams and stored data](../commands/scoreboard-and-data.md)
...
`NodeEvaluator` that scores blocks rather than reading them directly. → [AI](../entities/ai-goals-and-brains.md)
...
`PermissionCheck`, neither of which is an integer. → [Brigadier and commands](../commands/brigadier-and-commands.md)
...
optimistically. The ack is permission to apply that opinion, not a rollback. → [prediction and acknowledgement](../client/prediction-and-acks.md)
...
an immutable value. → [the frame](../rendering/the-frame.md), [entity rendering](../rendering/entity-rendering.md)
...
future tick, with a priority breaking ties inside that tick. → [block ticks and fluids](../world/block-ticks-and-fluids.md)

**Score** — one number for one holder under one objective, reached through a
`ScoreAccess` handle rather than a setter. → [scores, teams and stored data](../commands/scoreboard-and-data.md)

**Screen** — one full-window client UI with its own widget tree and
lifecycle; the server is told nothing about most of them. → [GUI and screens](../client/gui-and-screens.md)
...
engine's own storage, not on the section. → [chunk anatomy](../world/chunk-anatomy.md)
...
with no pool and no registry. → [hand-built structures](../worldgen/hand-built-structures.md)
...
is read by collision and by rendering. → [scores, teams and stored data](../commands/scoreboard-and-data.md)
...
rest. → [the server tick](../server/server-tick.md), [the client loop](../client/the-client-loop.md)
...
as a `WorldClock` advances. → [environment attributes and timelines](../world/environment-attributes-and-timelines.md)
...
this trigger. Nothing broadcasts. → [advancements](../commands/advancements.md)
...
through. → [the window](../rendering/the-window.md)

**World clock** — the counter a timeline is sampled against; there is more
than one, and they do not all advance at the same rate. → [environment attributes and timelines](../world/environment-attributes-and-timelines.md)
