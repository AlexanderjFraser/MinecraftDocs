...
The one sentence a player recognises: *stone takes 1.5 seconds with an
iron pickaxe, drops nothing without one, and the cracks you see on other
players' blocks lag behind theirs.*
...
- **The tool's side** is a data component: `DataComponents.TOOL`, a `Tool`
  ...
  `Tool.damagePerBlock` and `Tool.canDestroyBlocksInCreative`.
  `Tool.getMiningSpeed` and `Tool.isCorrectForDrops` take the first
  matching rule. `ToolMaterial.applyToolProperties` builds a pickaxe's
  component as *deny drops on `ToolMaterial.incorrectBlocksForDrops`*
  then *mine-and-drop `BlockTags.MINEABLE_WITH_PICKAXE` at the material's
  speed* — `ToolMaterial.IRON` is `BlockTags.INCORRECT_FOR_IRON_TOOL`,
  250 durability, speed 6.0. Swords get a `Tool` with
  `Tool.damagePerBlock` 2 and `Tool.canDestroyBlocksInCreative` false.
- **The player's side** is four synced attributes —
  ...
  `Attributes.BLOCK_INTERACTION_RANGE` — plus the effects
  `MobEffects.HASTE`, `MobEffects.CONDUIT_POWER` and `MobEffects.MINING_FATIGUE`
  read through `MobEffectUtil.hasDigSpeed` and
...
- **The cracks** everyone else sees: `BlockDestructionProgress` (id, pos,
  progress 0–9 — a class in `server/level` that ships in the server jar
  but is only used by the client), kept by `ClientLevel.destroyBlockProgress`
  in `ClientLevel.destroyingBlocks` (by breaker entity id) and
  `ClientLevel.destructionProgress` (by position, a sorted set whose last
  entry is the deepest crack). `LevelExtractor` turns those within 32
  blocks into `BlockBreakingRenderState`s each frame, drawn with
  `ModelBakery.DESTROY_TYPES` — Part X.
...
zero forever. `Player.getDestroySpeed` builds the player's speed from the
*selected hotbar item* (`Inventory.getSelectedItem`, not the main-hand
slot): `ItemStack.getDestroySpeed` → `Tool.getMiningSpeed` (6.0 for iron on
a pickaxe block, 1.0 otherwise); if that is above 1.0, **add**
`Attributes.MINING_EFFICIENCY`; multiply by 1 + 0.2 × (haste level + 1);
multiply by the mining-fatigue factor; multiply by
`Attributes.BLOCK_BREAK_SPEED`; multiply by `Attributes.SUBMERGED_MINING_SPEED`
if the eyes are in `FluidTags.WATER`; divide by five if not on the ground.
Iron pickaxe on stone: 6 ÷ 1.5 ÷ 30 = 0.133 per tick, so the eighth tick
crosses 1.0.
...
1. **Mouse down.** `Minecraft.startAttack`: `Minecraft.hitResult` is a
   ...
   `BlockBehaviour.BlockStateBase.attack` (nothing for stone), computes
   ...
   and writes stage −1 into `ClientLevel.destroyBlockProgress`. The
   ...
   (Creative predicts the break immediately and sends START alone.)
2. **The server starts its clock.** `ServerGamePacketListenerImpl.handlePlayerAction`
   ...
   `ServerLevel.mayInteract`; `ServerPlayerGameMode.destroyProgressStart`
   ...
   (N), flushed at the end of the tick as a `ClientboundBlockChangedAckPacket`
   with nothing to reconcile. Every refusal here is answered with a
   `ClientboundBlockUpdatePacket` of the true state.
3. **Seven silent ticks.** Client: `Minecraft.continueAttack` →
   ...
   `ClientLevel.destroyBlockProgress`, spawns one `TerrainParticle` via
   `ClientLevel.addBreakingBlockEffect`, swings. **No packets.** Server:
...
5. **The server checks.** `ServerPlayerGameMode.handleBlockBreakAction`
   ...
   it sets `ServerPlayerGameMode.hasDelayedDestroy`, keeps ticking that
   position from the original start, and breaks the block itself when
   its own clock reaches 1.0.
6. **The removal.** `ServerPlayerGameMode.destroyBlock`:
   ...
   `ItemStack.mineBlock` → `Item.mineBlock` → `ItemStack.hurtAndBreak` by
   `Tool.damagePerBlock` (unbreaking through `EnchantmentHelper.processDurabilityChange`,
...
8. **The item entity.** `Block.popResource`: `GameRules.BLOCK_DROPS`
   true → a new `ItemEntity` at a ±0.25 offset with a small random
   velocity, `ItemEntity.setDefaultPickUpDelay` (ten ticks),
...
- **The client sends nothing while digging.** START, then STOP or ABORT.
  The server's `ServerPlayerGameMode.incrementDestroyProgress` recomputes
  from `ServerPlayerGameMode.destroyProgressStart` every tick rather than
  accumulating, so a tool or effect change mid-dig rescales the whole dig
  retroactively on the server.
- **A too-early STOP is a deferral, not a rejection.** Below 0.7 the
  ...
  stands meanwhile, and new digs are ignored until then. Hard rejections
  are only reach, height, spawn protection, the border,
  `Player.blockActionRestricted`, `GameMasterBlock` and
  `ItemStack.canDestroyBlock` — each answered with a block update.
...
- **Efficiency and aqua affinity are attributes.** `Enchantments.EFFICIENCY`
  ...
  The 30/100 divisor and the ÷5 in the air are the last hard-coded
  constants in `BlockBehaviour.getDestroyProgress` and `Player.getDestroySpeed`.
- **One predicate, two consequences.** `Player.hasCorrectToolForDrops`
  ...
  at all; the tool takes durability either way. It reads
  `Inventory.getSelectedItem`, which is why `MultiPlayerGameMode.ensureHasSentCarriedItem`
  precedes every progress tick.
- **The loot key is fixed at construction.** `BlockBehaviour.Properties.effectiveDrops`
  ...
  sequence (`MinecraftServer.getRandomSequence`), not the level random.
