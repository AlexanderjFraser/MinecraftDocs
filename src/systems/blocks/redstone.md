...
block entity that both sides tick.
...
- **Signal** is an int 0–15 (`Redstone.SIGNAL_MIN`, `Redstone.SIGNAL_MAX`,
  ...
  *is the collision shape a full block*. The level reads them through
  ...
  (the block's own, or if it conducts, the strong power into it),
  ...
  for repeater and comparator sides.
...
Repeaters and comparators are scheduled ticks: `DiodeBlock.checkTickOnNeighbor`
books `ScheduledTickAccess.scheduleTick` at `TickPriority.HIGH`, `TickPriority.VERY_HIGH`
when turning off and `TickPriority.EXTREMELY_HIGH` when facing another
diode — the reason a repeater turns off before it turns on
...
sequenceDiagram
    ...
    CL->>PB: triggerEvent → the client's own moveBlocks
...
5. **The second dust.** The first of those reaches L+2 with the wire as
   source; the default evaluator does not skip wire-sourced updates.
...
7. **Can it push?** Not `PistonBaseBlock.EXTENDED` →
   ...
   stone — `PushReaction.NORMAL`, hardness not −1, no block entity, not
   obsidian, inside the border — then `PistonStructureResolver.addBlockLine`
...
9. **The move.** A second `PistonStructureResolver.resolve`; destroyed
   ...
   `PistonBaseBlock.EXTENDED` true with flags 67 (neighbours told *moved
   by piston*), `SoundEvents.PISTON_EXTEND`, `GameEvent.BLOCK_ACTIVATE`,
...
10. **The client simulates.** `ClientPacketListener.handleBlockEvent` →
    ...
    `PistonMovingBlockEntity`s, the sound played locally. The
    authoritative `ClientboundBlockUpdatePacket`s for the placeholders
    arrive a tick later through `ClientLevel.setServerVerifiedBlockState`
    and change nothing. `PistonHeadRenderer` draws
...
12. **Landing.** The tick after: progress at 1 → the server removes the
    ...
    same way (`PistonMovingBlockEntity.finalTick` is the early-exit form,
    also called from `BlockEntity.preRemoveSideEffects`). The client
...
only the wires that changed (flag 2 plus `Block.UPDATE_SKIP_SHAPE_UPDATE_ON_WIRE`,
which `NeighborUpdater.executeShapeUpdate` honours by skipping
wire-to-wire shape updates), and `ExperimentalRedstoneWireEvaluator.causeNeighborUpdates`
issues one `Level.neighborChanged` per connected side per changed wire
in `Orientation.getDirections` order. `RedStoneWireBlock.neighborChanged`
...
- **Called by:** every `Level.setBlock` with `Block.UPDATE_NEIGHBORS`,
  ...
  piston; scheduled ticks for `DiodeBlock.tick`.
...
- **Quasi-connectivity is a few lines in `PistonBaseBlock.getNeighborSignal`**:
  after the five sides it repeats `SignalGetter.hasSignal` for the
  neighbours of the position above. Nothing else implements it. The
  piston also asks `SignalGetter.hasSignal` downward on *itself*, which
  is how strong power into the piston works.
...
- **Dust points into a piston only by the line-completion rule** in
  `RedStoneWireBlock.getConnectionState` — pistons are non-conductors
  and non-sources.
...
- **Redstone torches burn out by history, not by state:**
  ...
  list; more than `RedstoneTorchBlock.MAX_RECENT_TOGGLES` (8) within
  `RedstoneTorchBlock.RECENT_TOGGLE_TIMER` (60) ticks and
  `RedstoneTorchBlock.isToggledTooFrequently` says so.
