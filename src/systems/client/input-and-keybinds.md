...
involves the tick. All of it has already happened before the tick that
observes it runs.
...
sequenceDiagram
    ...
    KH->>MC: handleGlobalKeyPress — fullscreen, screenshot, friends: these never become clicks
    ...
    MC->>KI: tick — reads isDown on seven mappings, not consumeClick
...
screen releases every mapping, closing one restores the toggles.
...
happened faster than the tick rate are not lost — and that a keybind read
from two places consumes twice. `Minecraft.handleKeybinds` is the drain, and
...
which is what screens do. `KeyMapping.same`, `KeyMapping.isDefault` and
`KeyMapping.isUnbound` are what the binding screen asks.

## The four bulk operations, and their single callers
...
mappings that want it. Four static operations walk them, and each has exactly
one caller.
...
drag handlers rather than the player; with the window unfocused nothing
...
of one edge with `Gui.setScreen`, guarded so the two cannot recurse; `MouseHandler.grabMouse`
also clears the screen. `MouseHandler.isMouseGrabbed` is the state,
`MouseHandler.setIgnoreFirstMove` suppresses the jump after a resize, and
`MouseHandler.simulateRightClick` exists for the macOS long-left-click quirk.
...
record, not an enum, so yes; registering a duplicate id throws. Ordering is a
fixed list of the built-in categories first, then registration order.

Not one class on this page sends a packet, which is worth saying out loud:
the intent a key press produces reaches the server much later and by an
entirely different route. The two smaller supporting types are
`ScrollWheelHandler`, the wheel's accumulator, and `InputType`, the
three-valued "what did the player last use" that decides whether focus rings
are drawn.
...
bottom. Then `ToggleKeyMapping` end to end, which is forty lines and explains
