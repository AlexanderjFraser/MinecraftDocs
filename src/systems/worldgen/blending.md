...
> Verified against **Minecraft 26.2** · Part XII · a chunk generated beside one an older version left behind: one measurement of the old ground, and the five places generation is bent around it.

In a world old enough to have been generated by a previous version, the debug
screen's chunk-generation entry reads *Blending: Old* on one side of a line and
stops reading it on the other. For roughly a hundred blocks either side of that
line the game is generating chunks that are not free to be whatever the seed
says — and directly against the seam they are hardly generated at all. The three
functions that shape overworld terrain, *offset*, *factor* and *jaggedness*, are
each swapped for a fixed target: a ground height the game read off the old
chunk's blocks a moment earlier, the constant ten, and zero. **At the seam the
overworld's terrain splines are not consulted.**
...
data packs. Here it is a function of the seed, the data packs, **and the blocks
in the old chunks around it** — which the game does not remember and has to go
and measure, one column at a time.
...
resolver returned as given. The lookup is cheap after the first time, because
the per-region bitset is memoised behind
`IOWorker.getOrCreateOldDataForRegion` and so is scanned once per region rather
than once per chunk; a development build can drop it entirely with the
`SharedConstants.DEBUG_DISABLE_BLENDING` flag, which short-circuits
`Blender.of`, the carving-mask filter and the border ticks alike.

## The cast

| class | what it decides | when |
|---|---|---|
| `Blender` | the three answers — the height alpha and offset, the blended density, and the biome override. The other two consumers are *static* methods on the same class and never ask an instance anything | built per step, used on the worldgen executor |
...
derives from the twenty-seven-cell height range. The cell here is the *quart* —
the four-by-four-by-four volume a biome is stored per
([biomes](biomes.md#deciding-a-chunks-biomes-cell-by-cell)) — and the
arithmetic is four quart cells per section across seven sections, less one, plus
three, converted back to chunks. It sits
...
columns in a ring round the chunk's edge — a column here being one horizontal
position sampled through the chunk's **old area**, the vertical band between the
minimum and maximum sections the old chunk actually had blocks in — measured out of that same old
...
### Height, as an alpha and an offset

`Blender.blendOffsetAndFactor` first checks whether the sample point sits exactly on a measured column; if it does,
...
### Density, as a lerp with a very short reach

`Blender.blendDensity` is called per sample from inside `NoiseChunk.BlendDensity`, the wrapper the
...
### Biome, as a replacement

`Blender.getBiomeResolver` wraps the biome source
in a `BiomeResolver` — the one-method interface the chunk's biome fill takes
([the trace: a chunk's biomes](biomes.md#deciding-a-chunks-biomes-cell-by-cell)) — that
...
Between them those three shapes are why the blend can still leave a visible
edge, in two different ways. A column whose old surface is none of the eleven
blocks the height scan recognises reports the bottom of the old area instead,
and that bogus height is averaged in with the rest. And because the biome is a
threshold rather than a ramp, the surface rules can change block for block
along a line while the terrain under them is still sloping.
