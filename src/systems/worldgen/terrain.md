...
builds a throwaway one-cell `NoiseChunk` with an empty `Blender` and the
...
density graph as three nodes, and `BelowZeroRetrogen` — the world-deepening
upgrade from four years ago — is still live code: it wraps the biome
