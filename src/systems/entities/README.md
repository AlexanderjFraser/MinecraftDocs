...
bookmark. They share one base class that is deliberately thin, one numbered
array for telling clients about themselves, one bag of named numbers, one
...
flowchart BT
    ...
    D -- "one of five channels that describe it" --> E
...
4. [Synched entity data](synched-entity-data.md) — a sheep is sheared and
   ...
   is decided by the order the JVM ran static initialisers in, and the packet
   stops at 254 because 255 means stop.
...
8. [Pathfinding](pathfinding.md) — the other half of the same lecture. Giving
   up is machinery: every node carries a timeout, and the mob you watch walk
   into a wall and then wander off is running a scheduled surrender.
9. [Damage and death](damage-and-death.md) — the part's closer. An arrow, a
   ...
   red flash is invisible: health drops and nothing else happens at all.
...
prediction ledger behind your own movement is Part X.
