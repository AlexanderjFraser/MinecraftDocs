...
6. [Enchanting](systems/items/enchanting.md) — the five paths that change
   what an item is enchanted with, one of which runs backwards. Watch it
   directly after *enchantments*.
...
1. [Player anatomy](systems/player/player-anatomy.md) — the vocabulary
   lecture: eight classes, two game-mode objects, forty-three slots. `Avatar`
   holds no instance state at all, and the main-hand item is not stored
   anywhere.
...
3. [Input to movement](systems/player/input-to-movement.md) — W is pressed,
   and the server decides whether to believe it. A movement key held for
   less than a tick never happened, and sending move packets faster makes
   the check stricter.
...
5. [The spear](systems/player/the-spear.md) — the 26.2 combat change, and
   ...
   packet with no target in it, and a charge that ignores the cooldown — and
   loses its ability to dismount long before it stops doing damage.
6. [Hunger and experience](systems/player/hunger-and-experience.md) — two
   ...
   costs exactly zero exhaustion, and the file of named thresholds behind it
   all is read by nothing.
7. [Status effects](systems/player/status-effects.md) — the closer. The
   client never runs one of an effect's hooks; it counts down and spawns
   particles, and an infinite effect is never re-sent at all.
