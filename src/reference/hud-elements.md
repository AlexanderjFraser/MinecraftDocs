...
The HUD is one ordered method, and every element in it is behind a condition.
...
| 14 | contextual bar | `ContextualBar.extractRenderState` | yes | always recorded, but which of four states it is in is re-decided every frame by `Hud.nextContextualInfoState` |
| 15 | experience level | `ContextualBar.extractExperienceLevel` | yes | the game mode has experience **and** the level is above zero — drawn inside the bar's pass, so it survives whichever bar wins |
| 16 | selected item name | `Hud.extractSelectedItemName` | yes | not a spectator, `Hud.toolHighlightTimer` above zero, and the stack is not empty |
| 17 | status effects | `Hud.extractEffects` | yes | the player has effects, no screen is showing them itself, and the instance sets `MobEffectInstance.showIcon` |
| 18 | boss bars | `BossHealthOverlay` | yes | the overlay has events |
| 19 | **sleep fade** | `Hud.extractSleepOverlay` | **no** | `Player.getSleepTimer` above zero — the one element between the two hidden-gated blocks |
| 20 | demo text | `Hud.extractDemoOverlay` | yes | `Minecraft.isDemo` |
| 21 | scoreboard sidebar | `Hud.displayScoreboardSidebar` | yes | a display objective for the team's colour slot, else for `DisplaySlot.SIDEBAR` |
| 22 | action bar | `Hud.extractOverlayMessage` | yes | `Hud.overlayMessageString` is set and its timer has not run out |
| 23 | title and subtitle | `Hud.extractTitle` | yes | `Hud.title` is set and `Hud.titleTime` is above zero |
| 24 | chat | `ChatComponent.extractRenderState` | yes | a player exists and the chat *screen* is not focused |
| 25 | tab list | `PlayerTabOverlay.extractRenderState` | yes | `Options.keyPlayerList` is down, and either this is not a local server, or more than one player is listed, or a `DisplaySlot.LIST` objective exists |
| 26 | subtitles | `SubtitleOverlay` | see note | deferred when there is no screen or the screen declares itself in-game UI — and recorded even while hidden, if a screen declaring itself in-game UI is up |
...
|---:|---|---|---|
| 27 | saving indicator | the frame is drawing a level | **no** |
| 28 | toasts | resources are loaded | checks the flag itself |
| 29 | debug overlay | the current screen is not `DebugOptionsScreen` | checks the flag itself |
| 30 | deferred subtitles | row 26 deferred them | — |
