...
drafted every page from the decompile (done; archived in `docs/pass1.md`);
**pass 2** — completeness and accuracy: every claim adversarially
fact-checked against the decompile, gaps filled, pages split and added
freely (length is pass 4's problem); **pass 3** — restructuring: each part
takes the shape of the system it explains, the diagram plan, the lecture
order drafted; **pass 4** — polish: wording, consistency, cuts; **pass 5+**
— the owner reads each page against the source, asks questions in the page,
and confirms the lecture order; then voice and cuts. Nothing is recorded
...
[docs/plan.md](docs/plan.md) — the pass roadmap (passes 2–5), the current
pass's charter, session protocol and schedule, and the session log.
**Read it first; tick it last.** [docs/pass2.md](docs/pass2.md) — the
...
confirmed by the owner in pass 5.
...
`src/lectures.md` (the lecture order, drafted in pass 3). `tools/deploy.sh` verifies, builds and deploys
