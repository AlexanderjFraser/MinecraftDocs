...
refinement, one lens each: **6 the lecture** (**next** — one page at a time:
the devices that became slots, the twin skeletons, the cuts), **7 the figures**
...
on 69 of them, in seven spellings and on every page of three parts, which is
pass 6's first job. What every page keeps: the
...
and ran sessions A–O on Opus, all fifteen done; `tools/check_deps.py` checks the landing
...
`parts.md` and the thirteen `part-<dir>.md` size phrases; the figure pipeline —
...
`tools/deploy.sh` regenerates the atlas and the eleven Reference views, runs
